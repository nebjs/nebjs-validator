...
const schemaProperties = function (props) {
  const {stack, parent = null, dataFrom = null, dataName = ''} = props, upSchemaPath = parent ? parent.schemaPath + '/' + parent.keyword : '';
  ...
  if (schemaFrom) {
    ...
    if (isArr || isObj) {
      ...
      if (data !== undefined) {
        ...
        if (parent) {
          dataPath = parent.dataPath ? parent.dataPath + '/' + parent.dataName : parent.dataName;
        } else {
          dataPath = '';
        }
      } else {
...
const validate = function (data) {
  this.data = data;
  const errors = [], stack = [], schemas = this.schemas;
