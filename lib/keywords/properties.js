...
const additionalPropertiesDataValid = function (schemasStack, dataStack) {
  ...
  switch (state) {
    case 0:
      ...
      if (addLen > 0) {
        const {schemas} = schemaStackItem, schema = schemas[0];
...
const patternPropertiesDataValid = function (schemasStack, dataStack) {
  const schemaStackItem = schemasStack[schemasStack.length - 1], dataStackItem = dataStack[schemaStackItem.dataIndex];
  const {data} = dataStackItem, {state, schemas} = schemaStackItem;
  let doIt;
  switch (state) {
    case 0:
      doIt = false;
...
const properties = [
  {name: 'additionalProperties', schema: {only: true, valid: {types: ['boolean', 'object']}}, data: {valid: additionalPropertiesDataValid}},
  {name: 'patternProperties', schema: {only: true, valid: {types: ['object'], value: patternPropertiesValid}}, data: {valid: patternPropertiesDataValid}},
  {name: 'properties', schema: {only: true, valid: {types: ['object']}}, data: {valid: propertiesDataValid}}];
