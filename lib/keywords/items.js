    ...
    case 0:
      ...
      if (dataLen > 0) {
        ...
        if (useSameItem) {
          ...
          const {schemas} = schemaStackItem, itemsLen = schemas.length, minLen = Math.min(dataLen, itemsLen);
...
const items = [
  {name: 'additionalItems', schema: {only: true, valid: {types: ['boolean', 'object']}}, data: {valid: additionalItemsDataValid}},
  {name: 'items', schema: {valid: {types: ['object']}}, data: {valid: itemsDataValid}}];
