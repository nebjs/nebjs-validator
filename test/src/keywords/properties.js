...
describe('验证器 关键字【properties、patternProperties、additionalProperties】的测试', function () {
  it('{"properties":{"foo":{"type":"string"},"bar":{"type":"number","minimum":2}}} ' +
    ...
    const validator = new ValidatorSchema({'properties': {'foo': {'type': 'string'}, 'bar': {'type': 'number', 'minimum': 2}}}, {});
  ...
  it('{"patternProperties":{"^fo.*$":{"type":"string"},"^ba.*$":{"type":"number"}}} ' +
    ...
    const validator = new ValidatorSchema({'patternProperties': {'^fo.*$': {'type': 'string'}, '^ba.*$': {'type': 'number'}}}, {});
    for (const v of valid) expect(validator.validate(v)).to.equal(true);
    for (const v of invalid) expect(validator.validate(v)).to.equal(false);
  });
  it('{"properties":{"foo":{"type":"number"}},"patternProperties":{"^.*r$":{"type":"number"}},"additionalProperties":false} ' +
    ...
    const validator = new ValidatorSchema({'properties':{'foo':{'type':'number'}},'patternProperties':{'^.*r$':{'type':'number'}},'additionalProperties':false}, {});
    for (const v of valid) expect(validator.validate(v)).to.equal(true);
    for (const v of invalid) expect(validator.validate(v)).to.equal(false);
  });
  it('{"properties":{"foo":{"type":"number"}},"patternProperties":{"^.*r$":{"type":"number"}},"additionalProperties":{"type":"string"}} ' +
    ...
    const validator = new ValidatorSchema({'properties':{'foo':{'type':'number'}},'patternProperties':{'^.*r$':{'type':'number'}},'additionalProperties':{'type':'string'}}, {});
    for (const v of valid) expect(validator.validate(v)).to.equal(true);
    for (const v of invalid) expect(validator.validate(v)).to.equal(false);
  });
  it('{"properties":{"foo":{"type":"number"}},"additionalProperties":false,"anyOf":[{"properties":{"bar":{"type":"number"}}},{"properties":{"baz":{"type":"number"}}}]} ' +
    ...
    const validator = new ValidatorSchema({'properties':{'foo':{'type':'number'}},'additionalProperties':false,'anyOf':[{'properties':{'bar':{'type':'number'}}},{'properties':{'baz':{'type':'number'}}}]}, {});
    for (const v of valid) expect(validator.validate(v)).to.equal(true);
    for (const v of invalid) expect(validator.validate(v)).to.equal(false);
  });
});
